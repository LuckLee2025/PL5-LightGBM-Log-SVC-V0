...
本脚本负责从网络上获取排列五的历史开奖数据，并将其保存为标准化的CSV文件。

主要功能:
1.  从文本文件 (pl5_asc.txt) 获取排列五历史数据。
2.  数据清理和格式化，确保数据质量。
3.  将数据保存到CSV文件 ('pl5.csv') 中。
4.  具备良好的错误处理和日志记录能力。

数据格式: Seq, pos_1, pos_2, pos_3, pos_4, pos_5
排列五规则: 5个数字，每个数字范围0-9，有顺序，允许重复
"""
...
# 网络数据源URL - 排列五数据源
TXT_DATA_URL = 'https://data.17500.cn/pl5_asc.txt'
...
logger = logging.getLogger('pl5_data_processor')
...
def fetch_pl5_data():
    """
    获取排列5数据并保存为pl5.csv，按Seq升序，去掉日期列
    
    基于提供的参考代码，结合现有项目的错误处理机制
    """
    logger.info("开始获取排列五数据...")
    ...
    for line_num, line in enumerate(lines, 1):
        ...
        try:
            ...
            # 如果数据包含日期，fields 应该至少有 6 个字段（Seq + 日期 + 5个数字）
            # 如果不包含日期，fields 应该至少有 6 个字段（Seq + 5个数字）
            if len(fields) < 6:
                logger.debug(f"跳过行{line_num}（字段不足6个）：{line}")
                continue
            ...
            if '-' in fields[1]:
                # 数据包含日期，数字从 fields[2] 开始
                if len(fields) < 7:
                    logger.debug(f"跳过行{line_num}（数字数量不足5个）：{line}")
                    continue
                numbers = fields[2:7]  # 提取5个数字
            else:
                # 数据不包含日期，数字从 fields[1] 开始
                numbers = fields[1:6]
                if len(numbers) != 5:
                    logger.debug(f"跳过行{line_num}（数字数量不足5个）：{line}")
                    continue
            ...
            try:
                ...
                if not all(0 <= num <= 9 for num in number_values):
    ...
    # 创建DataFrame
    df = pd.DataFrame(data, columns=['Seq'] + [f'pos_{i}' for i in range(1, 6)])
