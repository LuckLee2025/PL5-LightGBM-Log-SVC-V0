...
def get_latest_verification_result() -> Optional[Dict]:
    ...
    try:
        ...
        calc_file = os.path.join(script_dir, 'latest_pl5_calculation.txt')
                ...
                for j in range(i, min(i+20, len(lines))):
                    if lines[j].startswith('评估期号'):
                        ...
                        try:
                            ...
                            if len(number_str) == 5:
                                result['prize_numbers'] = [int(number_str[0]), int(number_str[1]), int(number_str[2]), int(number_str[3]), int(number_str[4])]
                        except:
                            pass
...
def send_analysis_report(report_content: str, period: int, recommendations: List[str], 
    ...
    Args:
        ...
        duplex_reference: 复式参考数据 {'pos1': [1,2,3,4,5], 'pos2': [0,1,4,6,9], 'pos3': [1,4,6,7,9], 'pos4': [2,5,7,8], 'pos5': [0,3,6,9]}
    
    Returns:
        推送结果字典
    """
    title = f"🎯 排列五第{period}期预测报告"
...
def send_error_notification(error_msg: str, script_name: str = "排列五系统") -> Dict:
    """发送错误通知
