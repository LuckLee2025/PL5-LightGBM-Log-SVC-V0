...
本脚本整合了统计分析、机器学习和策略化组合生成，为排列五彩票提供数据驱动的
...
# 原始排列五数据CSV文件路径 (由 pl5_data_processor.py 生成)
CSV_FILE_PATH = os.path.join(SCRIPT_DIR, 'pl5.csv')
# 预处理后的数据缓存文件路径，避免每次都重新计算特征
PROCESSED_CSV_PATH = os.path.join(SCRIPT_DIR, 'pl5_processed.csv')
...
# 排列五位数定义，用于特征工程和模式分析
POSITION_NAMES = ['位置1', '位置2', '位置3', '位置4', '位置5']
...
DEFAULT_WEIGHTS = {
    ...
    'TOP_N_NUMBERS_FOR_CANDIDATE': 25,  # 从0-9中选择评分最高的N个(排列五需要更多候选)
    ...
    # --- 组合多样性控制 ---
    'DIVERSITY_MIN_DIFFERENT_NUMBERS': 2,
}
...
# 主日志记录器
logger = logging.getLogger('pl5_analyzer')
logger.setLevel(logging.DEBUG)
logger.propagate = False

# 进度日志记录器
progress_logger = logging.getLogger('pl5_progress')
...
def get_prize_level(pos_1_hit: bool, pos_2_hit: bool, pos_3_hit: bool, pos_4_hit: bool, pos_5_hit: bool, is_direct: bool = True) -> Optional[str]:
    """
    根据排列五中奖情况确定奖级
    ...
    if is_direct:
        # 直选：五个位置都要对
        if pos_1_hit and pos_2_hit and pos_3_hit and pos_4_hit and pos_5_hit:
            return "直选"
        return None
    else:
        # 排列五没有组选玩法，只有直选
...
def clean_and_structure(df: pd.DataFrame) -> Optional[pd.DataFrame]:
    ...
    try:
        # 检查必要的列
        required_columns = ['Seq', 'pos_1', 'pos_2', 'pos_3', 'pos_4', 'pos_5']
        ...
        df['pos_1'] = pd.to_numeric(df['pos_1'], errors='coerce')
        df['pos_2'] = pd.to_numeric(df['pos_2'], errors='coerce')
        df['pos_3'] = pd.to_numeric(df['pos_3'], errors='coerce')
        df['pos_4'] = pd.to_numeric(df['pos_4'], errors='coerce')
        df['pos_5'] = pd.to_numeric(df['pos_5'], errors='coerce')
        ...
        # 验证数据范围
        valid_mask = (
            (df['pos_1'].between(0, 9)) &
            (df['pos_2'].between(0, 9)) &
            (df['pos_3'].between(0, 9)) &
            (df['pos_4'].between(0, 9)) &
            (df['pos_5'].between(0, 9)) &
            (df['Seq'] > 0)
        )
        df = df[valid_mask]
...
def feature_engineer(df: pd.DataFrame) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        # 基础特征
        df['sum_all'] = df['pos_1'] + df['pos_2'] + df['pos_3'] + df['pos_4'] + df['pos_5']  # 五位数和值
        df['span'] = df[['pos_1', 'pos_2', 'pos_3', 'pos_4', 'pos_5']].max(axis=1) - df[['pos_1', 'pos_2', 'pos_3', 'pos_4', 'pos_5']].min(axis=1)  # 跨度
        
        # 奇偶特征
        df['odd_count'] = (df['pos_1'] % 2) + (df['pos_2'] % 2) + (df['pos_3'] % 2) + (df['pos_4'] % 2) + (df['pos_5'] % 2)  # 奇数个数
        df['even_count'] = 5 - df['odd_count']  # 偶数个数
        
        # 大小特征（0-4为小，5-9为大）
        df['big_count'] = (df['pos_1'] >= 5).astype(int) + (df['pos_2'] >= 5).astype(int) + (df['pos_3'] >= 5).astype(int) + (df['pos_4'] >= 5).astype(int) + (df['pos_5'] >= 5).astype(int)
        df['small_count'] = 5 - df['big_count']
        
        # 质合特征（2,3,5,7为质数，其余为合数）
        prime_numbers = {2, 3, 5, 7}
        df['prime_count'] = 0
        for col in ['pos_1', 'pos_2', 'pos_3', 'pos_4', 'pos_5']:
            df['prime_count'] += df[col].isin(prime_numbers).astype(int)
        df['composite_count'] = 5 - df['prime_count']
        
        # 重复数字特征
        df['unique_count'] = df[['pos_1', 'pos_2', 'pos_3', 'pos_4', 'pos_5']].nunique(axis=1)  # 不重复数字个数
        df['repeat_count'] = 5 - df['unique_count']  # 重复数字个数
        
        # 连号特征
        def count_consecutive(row):
            numbers = sorted([row['pos_1'], row['pos_2'], row['pos_3'], row['pos_4'], row['pos_5']])
            consecutive = 0
        ...
        def get_form_type(row):
            numbers = [row['pos_1'], row['pos_2'], row['pos_3'], row['pos_4'], row['pos_5']]
            unique_nums = set(numbers)
            unique_count = len(unique_nums)
            
            if unique_count == 5:
                return 'all_different'  # 五个数字都不同
            elif unique_count == 4:
                return 'one_pair'  # 有一对相同数字
            elif unique_count == 3:
                # 可能是两对或者三个相同
                count_dict = {}
                for num in numbers:
                ...
                if max_count == 3:
                    return 'three_same'  # 三个相同数字
                else:
                    return 'two_pairs'  # 两对相同数字
            elif unique_count == 2:
                count_dict = {}
                for num in numbers:
                    count_dict[num] = count_dict.get(num, 0) + 1
                max_count = max(count_dict.values())
                if max_count == 4:
                    return 'four_same'  # 四个相同数字
                else:
                    return 'full_house'  # 三个+两个相同
            else:
                return 'five_same'  # 五个数字相同
...
def analyze_frequency_omission(df: pd.DataFrame) -> dict:
    ...
    try:
        ...
        # 分析每个位置的号码频率和遗漏
        for pos_idx, pos_name in enumerate(['pos_1', 'pos_2', 'pos_3', 'pos_4', 'pos_5'], 1):
            pos_analysis = {}
    ...
    try:
        ...
        if 'sum_all' in df.columns:
            ...
            result['sum_patterns'] = {
                ...
                'most_common_range': (20, 25),  # 排列五和值最常见范围
        ...
        if 'span' in df.columns:
            ...
            result['span_patterns'] = {
                ...
                'most_common': span_distribution.idxmax() if not span_distribution.empty else 5,
...
def analyze_associations(df: pd.DataFrame, weights_config: Dict) -> pd.DataFrame:
    ...
    try:
        ...
        for _, row in df.iterrows():
            transaction = [f"pos1_{int(row['pos_1'])}", f"pos2_{int(row['pos_2'])}", f"pos3_{int(row['pos_3'])}", f"pos4_{int(row['pos_4'])}", f"pos5_{int(row['pos_5'])}"]
        ...
        # 应用Apriori算法
        min_support = weights_config.get('ARM_MIN_SUPPORT', 0.02)
        frequent_itemsets = apriori(df_encoded, min_support=min_support, use_colnames=True)
        
        if frequent_itemsets.empty:
            logger.warning("未发现满足最小支持度的频繁项集")
            return pd.DataFrame()
        
        # 生成关联规则
        min_confidence = weights_config.get('ARM_MIN_CONFIDENCE', 0.60)
        try:
        ...
        if rules.empty:
            logger.warning("未发现满足最小置信度的关联规则")
            return pd.DataFrame()
...
def calculate_scores(freq_data: Dict, probabilities: Dict, weights: Dict) -> Dict[str, Dict[int, float]]:
    ...
    try:
        ...
        for pos_name in ['pos_1', 'pos_2', 'pos_3', 'pos_4', 'pos_5']:
            position_scores = {}
...
def train_prediction_models(df_train_raw: pd.DataFrame, ml_lags_list: List[int]) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        feature_columns = [col for col in df_with_lags.columns 
                          if col not in ['Seq', 'pos_1', 'pos_2', 'pos_3', 'pos_4', 'pos_5', 'form_type']]
...
def predict_next_draw_probabilities(df_historical: pd.DataFrame, trained_models: Optional[Dict], ml_lags_list: List[int]) -> Dict[str, Dict[int, float]]:
    ...
    try:
        if trained_models is None:
            ...
            return {pos_name: {num: 0.1 for num in NUMBER_RANGE} for pos_name in ['pos_1', 'pos_2', 'pos_3', 'pos_4', 'pos_5']}
...
def generate_combinations(scores_data: Dict, pattern_data: Dict, arm_rules: pd.DataFrame, weights_config: Dict) -> Tuple[List[Dict], List[str]]:
    ...
    try:
        # 获取各位置的候选号码
        candidates = {}
        top_n = weights_config.get('TOP_N_NUMBERS_FOR_CANDIDATE', 25)
        ...
        # 生成所有可能的组合
        all_combinations = []
        for pos1 in candidates['pos_1']:
            for pos2 in candidates['pos_2']:
                for pos3 in candidates['pos_3']:
                    for pos4 in candidates['pos_4']:
                        for pos5 in candidates['pos_5']:
                            combination = [pos1, pos2, pos3, pos4, pos5]
                            
                            # 计算组合得分
                            combo_score = (
                                scores_data.get('pos_1', {}).get(pos1, 0) +
                                scores_data.get('pos_2', {}).get(pos2, 0) +
                                scores_data.get('pos_3', {}).get(pos3, 0) +
                                scores_data.get('pos_4', {}).get(pos4, 0) +
                                scores_data.get('pos_5', {}).get(pos5, 0)
                            )
...
def calculate_arm_bonus(combination: List[int], arm_rules: pd.DataFrame, weights_config: Dict) -> float:
    ...
    try:
        ...
        # 构建当前组合的项集
        current_items = {f"pos1_{combination[0]}", f"pos2_{combination[1]}", f"pos3_{combination[2]}", f"pos4_{combination[3]}", f"pos5_{combination[4]}"}
        ...
        for _, rule in arm_rules.iterrows():
            ...
            # 检查规则是否匹配
            if antecedents.issubset(current_items) and consequents.issubset(current_items):
                rule_bonus = (
                    weights_config.get('ARM_COMBINATION_BONUS_WEIGHT', 15.0) +
                    rule['lift'] * weights_config.get('ARM_BONUS_LIFT_FACTOR', 0.50) +
                    rule['confidence'] * weights_config.get('ARM_BONUS_CONF_FACTOR', 0.30)
                )
                bonus += rule_bonus
...
def run_analysis_and_recommendation(df_hist: pd.DataFrame, ml_lags: List[int], weights_config: Dict, arm_rules: pd.DataFrame) -> Tuple:
    ...
    probabilities = predict_next_draw_probabilities(df_hist, ml_models, ml_lags) if ml_models else {'pos_1': {}, 'pos_2': {}, 'pos_3': {}, 'pos_4': {}, 'pos_5': {}}
...
def run_backtest(full_df: pd.DataFrame, ml_lags: List[int], weights_config: Dict, arm_rules: pd.DataFrame, num_periods: int) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    for i in range(num_periods):
        ...
        actual_numbers = [actual_outcome['pos_1'], actual_outcome['pos_2'], actual_outcome['pos_3'], actual_outcome['pos_4'], actual_outcome['pos_5']]
        ...
        if not predicted_combos:
            ...
            for combo_dict in predicted_combos:
                ...
                # 直选验证（位置完全匹配）
                direct_hits = sum(1 for j in range(5) if combo[j] == actual_numbers[j])
                direct_prize = get_prize_level(combo[0] == actual_numbers[0], 
                                               combo[1] == actual_numbers[1], 
                                               combo[2] == actual_numbers[2],
                                               combo[3] == actual_numbers[3],
                                               combo[4] == actual_numbers[4], 
                                               is_direct=True)
                
                # 排列五只有直选玩法
                best_prize = direct_prize
                if best_prize:
            ...
            best_hits_per_period.append({
                ...
                'prize': get_prize_level(period_best_combo[0] == actual_numbers[0] if period_best_combo else False,
                                        ...
                                        period_best_combo[3] == actual_numbers[3] if period_best_combo else False,
                                        period_best_combo[4] == actual_numbers[4] if period_best_combo else False,
...
def objective(trial: optuna.trial.Trial, df_for_opt: pd.DataFrame, ml_lags: List[int], arm_rules: pd.DataFrame) -> float:
    ...
    # 定义一个分数来衡量表现，排列五直选奖金为100000元
    prize_weights = {'直选': 100000}
...
def main():
    ...
    log_filename = os.path.join(SCRIPT_DIR, f"pl5_analysis_output_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
    ...
    if not backtest_results_df.empty:
        ...
        logger.info("\n--- 1. 奖金与回报分析 ---")
        prize_dist, prize_values = backtest_stats.get('prize_counts', {}), {'直选': 100000}
        ...
        logger.info("\n--- 2. 核心性能指标 ---")
        logger.info(f"  - 平均直选命中 (每注): {backtest_results_df['direct_hits'].mean():.3f} / 5")
        logger.info(f"  - 平均总命中 (每注): {backtest_results_df['total_hits'].mean():.3f} / 5")
        logger.info("\n--- 3. 每期最佳命中表现 ---")
        if (best_hits_df := backtest_stats.get('best_hits_per_period')) is not None and not best_hits_df.empty:
            ...
            logger.info(f"    - 直选(五位全对): {direct_hits} / {num_periods_tested} 期")
            logger.info(f"    注：排列五只有直选玩法，需要五个位置的数字和顺序全部正确")
    ...
    logger.info("\n--- 复式参考 ---")
    if final_scores and final_scores.get('pos_1'):
        # 获取各位置的高分号码
        top_pos1 = sorted([n for n, _ in sorted(final_scores['pos_1'].items(), key=lambda x: x[1], reverse=True)[:5]])
        top_pos2 = sorted([n for n, _ in sorted(final_scores['pos_2'].items(), key=lambda x: x[1], reverse=True)[:5]])
        top_pos3 = sorted([n for n, _ in sorted(final_scores['pos_3'].items(), key=lambda x: x[1], reverse=True)[:5]])
        top_pos4 = sorted([n for n, _ in sorted(final_scores['pos_4'].items(), key=lambda x: x[1], reverse=True)[:5]])
        top_pos5 = sorted([n for n, _ in sorted(final_scores['pos_5'].items(), key=lambda x: x[1], reverse=True)[:5]])
        logger.info(f"  万位 (Top 5): {' '.join(str(n) for n in top_pos1)}")
        logger.info(f"  千位 (Top 5): {' '.join(str(n) for n in top_pos2)}")
        logger.info(f"  百位 (Top 5): {' '.join(str(n) for n in top_pos3)}")
        logger.info(f"  十位 (Top 5): {' '.join(str(n) for n in top_pos4)}")
        logger.info(f"  个位 (Top 5): {' '.join(str(n) for n in top_pos5)}")
    
    logger.info("\n" + "="*60 + f"\n--- 报告结束 (详情请查阅: {os.path.basename(log_filename)}) ---\n")
    
    # 9. 微信推送
    try:
        from pl5_wxpusher import send_analysis_report, send_wxpusher_message_fallback
        ...
        if push_result.get('success'):
            ...
            try:
                # 构建简化的推送内容
                simple_content = f"🎯 排列五第{last_period + 1}期预测报告\n\n📋 推荐号码:\n"
                ...
                fallback_result = send_wxpusher_message_fallback(
                    ...
                    title=f"🎯 排列五第{last_period + 1}期预测 (备用推送)"
                )
    ...
    # 10. 更新latest_pl5_analysis.txt
    try:
        latest_file_path = os.path.join(SCRIPT_DIR, 'latest_pl5_analysis.txt')
        ...
        if os.path.exists(latest_file_path):
            ...
            logger.info(f"已更新 latest_pl5_analysis.txt (大小: {final_file_size} 字节)")
            ...
            if os.getenv('CI') or os.getenv('GITHUB_ACTIONS'):
                ...
                try:
                    with open(latest_file_path, 'r', encoding='utf-8') as f:
                        ...
                        logger.info(f"latest_pl5_analysis.txt 前10行内容:")
                        for i, line in enumerate(lines, 1):
                            logger.info(f"第{i}行: {line.strip()}")
                except Exception as debug_e:
                    logger.error(f"读取文件用于调试失败: {debug_e}")
        else:
            logger.error("latest_pl5_analysis.txt 文件创建失败")
            
    except Exception as e:
        logger.error(f"更新 latest_pl5_analysis.txt 失败: {e}")
        # 尝试创建一个最小的报告文件
        try:
            emergency_path = os.path.join(SCRIPT_DIR, 'latest_pl5_analysis.txt')
            with open(emergency_path, 'w', encoding='utf-8') as f:
                f.write(f"排列五分析报告 (紧急模式)\n")
                f.write(f"生成时间: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
                f.write(f"预测期号: 第{last_period + 1}期\n")
...
def save_analysis_report(df: pd.DataFrame, freq_data: Dict, pattern_data: Dict, 
    ...
    try:
        ...
        report_file = os.path.join(SCRIPT_DIR, f"pl5_analysis_output_{timestamp}.txt")
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            ...
            for i, combo in enumerate(recommendations):
                f.write(f"注 {i+1}: [{combo['numbers'][0]}, {combo['numbers'][1]}, {combo['numbers'][2]}, {combo['numbers'][3]}, {combo['numbers'][4]}]\n")
